...
(function ($) {
    ...
    function kronos(element, settings) {
      ...
      _.markups = {
        ...
        year: {
          ...
          prev:
            '<button type="button" class="kronos-year-prev" title="' +
            _.options.text.btnYearPrev +
            '">' +
            _.options.text.btnYearPrev +
            "</button>",
          next:
            '<button type="button" class="kronos-year-next" title="' +
            _.options.text.btnYearNext +
            '">' +
            _.options.text.btnYearNext +
            "</button>",
        },
        month: {
          ...
          prev:
            '<button type="button" class="kronos-month-prev" title="' +
            _.options.text.btnMonthPrev +
            '">' +
            _.options.text.btnMonthPrev +
            "</button>",
          next:
            '<button type="button" class="kronos-month-next" title="' +
            _.options.text.btnMonthNext +
            '">' +
            _.options.text.btnMonthNext +
            "</button>",
        },
        date: {
          ...
          prev:
            '<button type="button" class="kronos-date-prev" title="' +
            _.options.text.btnDatePrev +
            '">' +
            _.options.text.btnDatePrev +
            "</button>",
          next:
            '<button type="button" class="kronos-date-next" title="' +
            _.options.text.btnDateNext +
            '">' +
            _.options.text.btnDateNext +
            "</button>",
  ...
  Kronos.prototype.convertFormat = function (core) {
    var _ = this,
      ...
      d;
    y =
      _.initial.formatY.length === 2
        ? String(core.substring(2, 4))
        : String(core.substring(0, 4));
    m = String(core.substring(4, 6));
    d = String(core.substring(6, 8));
    f = _.options.format
      .replace(_.initial.formatY, y)
      .replace(_.initial.formatM, m)
      .replace(_.initial.formatD, d);
    return f;
  };
  ...
  Kronos.prototype.slideDatepicker = function (
    delta,
    target1,
    target2,
    callback
  ...
  Kronos.prototype.visibleMotion = function (type, show, hide) {
    var class1 =
      type === "in" ? "kronos-outer-scale-h" : "kronos-outer-scale-r";
    var class2 =
      type === "in" ? "kronos-outer-scale-r" : "kronos-outer-scale-h";
  ...
  Kronos.prototype.checkFormat = function () {
    ...
    _.initial.formatY = String(
      _.options.format.substring(_.initial.indexYS, _.initial.indexYE)
    );
    _.initial.formatM = String(
      _.options.format.substring(_.initial.indexMS, _.initial.indexME)
    );
    _.initial.formatD = String(
      _.options.format.substring(_.initial.indexDS, _.initial.indexDE)
    );
  };
  ...
  Kronos.prototype.getTodayDate = function () {
    ...
    _.corevar.today =
      String(_.initial.todayY) +
      _.combineZero(_.initial.todayM + 1) +
      _.combineZero(_.initial.todayD);
  };

  Kronos.prototype.setLayout = function () {
    var _ = this;
    _.element.common.outer = _.element.kronos
      .wrap(_.markups.common.outer)
      .parent(".kronos-outer");
    _.element.common.inner = _.element.common.outer
      .append(_.markups.common.inner)
      .children(".kronos-inner");
    _.element.year.outer = _.element.common.inner
      .append(_.markups.year.outer)
      .children(".kronos-year-outer");
    _.element.month.outer = _.element.common.inner
      .append(_.markups.month.outer)
      .children(".kronos-month-outer");
    _.element.date.outer = _.element.common.inner
      .append(_.markups.date.outer)
      .children(".kronos-date-outer");
  };
  ...
  Kronos.prototype.setKronosValue = function () {
    ...
    if (_.options.initDate) {
      ...
      _.element.kronos
        .attr({ core: _.corevar.selectedC })
        .val(_.corevar.selectedF);
    }
  };
  ...
  Kronos.prototype.openDatepicker = function () {
    // ë‹¬ë ¥ ì—´ê¸°
    var _ = this;
    _.setDateInit(0); // ë°ì´íŠ¸í”¼ì»¤ í•˜ìœ„ ë§ˆí¬ì—…
    _.element.year.outer.addClass("kronos-outer-scale-h");
    _.element.month.outer
      .addClass("kronos-outer-scale-h")
      .removeClass("kronos-outer-scale-r");
  ...
  Kronos.prototype.setThisDate = function () {
    ...
    _.initial.prevY =
      _.initial.thisM === 0 ? _.initial.thisY - 1 : _.initial.thisY;
    _.initial.prevM = _.initial.thisM === 0 ? 11 : _.initial.thisM - 1;
    _.initial.nextY =
      _.initial.thisM === 11 ? _.initial.thisY + 1 : _.initial.thisY;
    _.initial.nextM = _.initial.thisM === 11 ? 0 : _.initial.thisM + 1;
    _.initial.dateLeng[1] =
      (_.initial.thisY % 4 === 0 && _.initial.thisY % 100 !== 0) ||
      _.initial.thisY % 400 === 0
        ? 29
        : 28;
  ...
  Kronos.prototype.setDateInit = function (delta) {
    ...
    _.setThisDate(); // ì˜¤ëŠ˜ ë‚ ì§œ ë°ì´í„° ì„¤ì •
    _.element.date.inner = _.element.date.outer
      .append(_.markups.date.inner)
      .children(".kronos-date-inner:last-child");
    _.element.date.head = _.element.date.inner
      .append(_.markups.date.head)
      .children(".kronos-date-head");
    _.element.date.body = _.element.date.inner
      .append(_.markups.date.body)
      .children(".kronos-date-body");
    _.element.date.prev = _.element.date.head
      .append(_.markups.date.prev)
      .children(".kronos-date-prev");
    _.element.date.title = _.element.date.head
      .append(_.markups.date.title)
      .children(".kronos-date-title");
    _.element.date.year = _.element.date.title
      .append(
        '<span class="kronos-date-year">' +
          _.initial.thisY +
          _.options.text.thisYear +
          "</span>"
      )
      .children(".kronos-date-year");
    _.element.date.month = _.element.date.title
      .append(
        '<span class="kronos-date-month">' +
          _.options.text.month[_.initial.thisM] +
          _.options.text.thisMonth +
          "</span>"
      )
      .children(".kronos-date-month");
    _.element.date.next = _.element.date.head
      .append(_.markups.date.next)
      .children(".kronos-date-next");
    _.element.date.inner.css(_.autoPrefixer(delta * 100));
    _.setDayMarkup(); // ìš”ì¼ ë§ˆí¬ì—… ê·¸ë¦¬ê¸°
    _.setDateMarkup(); // ê° ë‚ ì§œ ë§ˆí¬ì—… ê·¸ë¦¬ê¸°
    _.getCoreData(); // ì½”ì–´ ë°ì´íƒ€ (this, from, to) ê°€ì ¸ì˜¤ê¸°
    _.setDateClasses(); // ê° ë‚ ì§œì— í´ëž˜ìŠ¤ ë§¤í•‘
    _.slideDatepicker(
      delta,
      _.element.date.inner,
      _.element.date.inner2,
      function () {
  ...
  Kronos.prototype.setMonthInit = function (delta) {
    var _ = this;
    _.element.month.inner = _.element.month.outer
      .append(_.markups.month.inner)
      .children(".kronos-month-inner:last-child");
    _.element.month.head = _.element.month.inner
      .append(_.markups.month.head)
      .children(".kronos-month-head");
    _.element.month.body = _.element.month.inner
      .append(_.markups.month.body)
      .children(".kronos-month-body");
    _.element.month.prev = _.element.month.head
      .append(_.markups.month.prev)
      .children(".kronos-month-prev");
    _.element.month.next = _.element.month.head
      .append(_.markups.month.next)
      .children(".kronos-month-next");
    _.element.month.inner.css(_.autoPrefixer(delta * 100));
    _.slideDatepicker(
      delta,
      _.element.month.inner,
      _.element.month.inner2,
      function () {
    ...
    _.element.month.title = _.element.month.head
      .append(_.markups.month.title)
      .children(".kronos-month-title")
      .text(_.initial.thisY + _.options.text.thisYear);
    // onEvents
  ...
  Kronos.prototype.setYearInit = function (delta) {
    ...
    _.element.year.inner = _.element.year.outer
      .append(_.markups.year.inner)
      .children(".kronos-year-inner:last-child");
    _.element.year.head = _.element.year.inner
      .append(_.markups.year.head)
      .children(".kronos-year-head");
    _.element.year.body = _.element.year.inner
      .append(_.markups.year.body)
      .children(".kronos-year-body");
    _.element.year.prev = _.element.year.head
      .append(_.markups.year.prev)
      .children(".kronos-year-prev");
    _.element.year.next = _.element.year.head
      .append(_.markups.year.next)
      .children(".kronos-year-next");
    _.element.year.inner.css(_.autoPrefixer(delta * 100));
    _.initial.startY = _.initial.thisY - 4 < 0 ? 0 : _.initial.thisY - 4;
    _.initial.endY = _.initial.startY + 8;
    _.slideDatepicker(
      delta,
      _.element.year.inner,
      _.element.year.inner2,
      function () {
    ...
    _.element.year.title = _.element.year.head
      .append(_.markups.year.title)
      .children(".kronos-year-title")
      .text(
        _.initial.startY +
          _.options.text.thisYear +
          " ~ " +
  ...
  Kronos.prototype.setDateMarkup = function () {
    var _ = this;
    _.initial.prevMonthFirstDate =
      _.initial.dateLeng[_.initial.prevM] - _.initial.thisDay;
    _.markups.date.content += "<tbody><tr>";
    for (var i = 1; i <= _.initial.thisDay; i++) {
      // ì´ì „ë‹¬ ë§ˆí¬ì—…
      _.markups.date.content +=
        '<td><button type="button" core="' +
        _.strings.prevYM +
        (_.initial.prevMonthFirstDate + i) +
        '" title="' +
        _.convertFormat(_.strings.prevYM + (_.initial.prevMonthFirstDate + i)) +
        '">' +
        (_.initial.prevMonthFirstDate + i) +
        "</button></td>";
      _.initial.weekCount++;
    ...
    for (var i = 1; i <= _.initial.thisDateLeng; i++) {
      ...
      _.markups.date.content +=
        ...
        _.combineZero(i) +
        '" title="' +
        _.convertFormat(_.strings.thisYM + _.combineZero(i)) +
        '">' +
        i +
        "</button></td>";
      _.initial.weekCount++;
    ...
    for (var i = 1; _.initial.weekCount != 0; i++) {
      ...
      if (_.initial.weekCount === 7) {
        ...
        _.markups.date.content +=
          ...
          _.combineZero(i) +
          '" title="' +
          _.convertFormat(_.strings.nextYM + _.combineZero(i)) +
          '">' +
          i +
          "</button></td>";
        _.initial.weekCount++;
  ...
  Kronos.prototype.setDateClasses = function () {
    ...
    _.element.date.date.each(function () {
      ...
      this.Class =
        this.index === 0 ? "sunday" : this.index === 6 ? "satday" : ""; // weekend
      this.Class +=
        this.core === _.corevar.selectedC ||
      ...
      this.Class +=
        _.isolateCore(this.core, "year+month") === _.strings.prevYM ||
        ...
        _.options.date.disabled.indexOf(this.mmdd) !== -1
          ...
          : "";
      this.Class +=
        _.options.date.holiday.indexOf(this.core) !== -1 ||
  ...
  Kronos.prototype.setYearMarkup = function (delta) {
    var _ = this;
    _.initial.startY =
      _.initial.startY + 9 * delta < 0 ? 0 : _.initial.startY + 9 * delta;
    _.initial.thisY = _.initial.thisY + 9 * delta;
    _.initial.endY = _.initial.startY + 8;
    for (var i = _.initial.startY; i <= _.initial.endY; i++) {
      _.element.year.body.append(
        '<button core="' +
          i +
          '" class="' +
          (i === _.initial.date.getFullYear() ? "selected" : "") +
          '">' +
          i +
          "</button>"
      );
    ...
    return {
      ...
      close: function (_) {
        $(document).on(_.events.click, function (e) {
          if (
            !$(e.target).closest(_.element.common.outer).length &&
            !_.options.visible
          ) {
            _.closeDatepicker();
          }
